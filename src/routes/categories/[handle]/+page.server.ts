...
import { getProductCategoryByHandle, listAllProductCategories, listProductsByCategoryIds } from '$lib/medusa';

function buildSubTree(all: HttpTypes.StoreProductCategory[], parentId: string): any[] {
...
export const load: PageServerLoad = async ({ params }) => {
	...
	const collectIds = (node: any): string[] => [node.id, ...(node.children?.flatMap(collectIds) ?? [])]
	const allIds = [category.id, ...subcategories.flatMap(collectIds)]
	const { products, count } = await listProductsByCategoryIds(allIds, { limit: 24, offset: 0, fields: 'title,handle,thumbnail,variants.id,*variants.calculated_price' })
