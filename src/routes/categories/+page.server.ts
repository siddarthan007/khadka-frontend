...
export const load: PageServerLoad = async () => {
	...
	const topLevel: Array<{ name: string; handle: string; thumbnail?: string | null; count?: number }>
		= tree.map((c) => ({ name: c.name!, handle: c.handle!, thumbnail: (c as any).metadata?.thumbnail ?? null }));
	return {
		tree,
		topLevel
	};
