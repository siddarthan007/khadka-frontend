<script lang="ts">
	...
	let { data }: { data?: { products: any[]; count: number; limit: number; offset: number; page: number; q?: string; categories?: any[]; collections?: any[] } } = $props();
	...
	$effect(() => { if (typeof document !== 'undefined') document.title = 'Products • KhadkaFoods' })
	...
	async function applyFilters(reset = true) {
		...
		try {
			...
			if (reset) { products = json.products ?? []; offset = products.length } else { products = [...products, ...(json.products ?? [])]; offset = products.length }
			count = json.count ?? count
		} finally { loading = false }
	}

	function resetFilters() { selectedCategories = []; selectedCollections = []; priceMin = 0; priceMax = 100; applyFilters(true) }

	function toggleCategory(id: string, checked: boolean) {
		selectedCategories = checked ? [...selectedCategories, id] : selectedCategories.filter((x) => x !== id)
	}
...
<svelte:head>
	<title>Products • KhadkaFoods</title>
	<meta name="description" content={`Browse ${count} products at KhadkaFoods.`} />
									...
									<li>
										<details class="group">
											<summary class="flex items-center gap-2 cursor-pointer">
												...
												<input type="checkbox" checked={selectedCategories.includes(cat.id)} onchange={(e)=>toggleCategory(cat.id,(e.target as HTMLInputElement).checked)} />
												<span>{cat.name}</span>
											</summary>
													...
													{#each cat.children as child}
														<li>
															<details class="group">
																<summary class="flex items-center gap-2 cursor-pointer">
																	<ChevronRight class="size-3 transition-transform group-open:rotate-90" />
																	<input type="checkbox" checked={selectedCategories.includes(child.id)} onchange={(e)=>toggleCategory(child.id,(e.target as HTMLInputElement).checked)} />
																		...
																		{#each child.children as gchild}
																			<li>
																				<label class="flex items-center gap-2 cursor-pointer">
																					<input type="checkbox" checked={selectedCategories.includes(gchild.id)} onchange={(e)=>toggleCategory(gchild.id,(e.target as HTMLInputElement).checked)} />
					...
					<div>
						...
						<div class="px-2">
							<DoubleRange
								...
								minGap={1}
								onInput={(v)=>{ priceMin = v[0]; priceMax = v[1]; }}
								onChange={(v)=>{ priceMin = v[0]; priceMax = v[1]; }}
							/>
						</div>
					</div>
					<div class="flex gap-2 pt-2">
						<button class="btn btn-primary btn-sm" onclick={() => applyFilters(true)} disabled={loading} class:loading={loading}>Apply</button>
			...
			<div class="lg:col-span-9">
				...
				{:else}
					<div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-4">
						{#each products as p}
							<ProductCard href={`/products/${p.handle}`} title={p.title} image={p.thumbnail ?? p.images?.[0]?.url ?? ''} price={(p?.variants ?? []).map((v:any)=>v?.calculated_price).filter(Boolean).reduce((min:any,cur:any)=>((cur?.calculated_amount ?? cur?.amount ?? 0)<(min?.calculated_amount ?? min?.amount ?? 0)?cur:min), (p?.variants?.[0]?.calculated_price ?? null))} variantId={p?.variants?.[0]?.id ?? null} createdAt={p?.created_at ?? null} />
						{/each}
					</div>
				{/if}
				{#if products.length < count}
					<div class="mt-6 flex justify-center">
						<button class="btn btn-primary" onclick={() => applyFilters(false)} disabled={loading} class:loading={loading}>Load more</button>
					</div>
				{/if}
			</div>
		</div>
