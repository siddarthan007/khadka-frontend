<script lang="ts">
  ...
  async function hydrate(_id:string){ /* already full from lookup */ }
  async function reorder(){
    if(!order) return; 
    try { 
      const store = getStoreClient() as any; 
      if(!store) return; 
      await store.store.cart.create({ items: order.items.map((i:any)=>({variant_id:i.variant_id, quantity:i.quantity}))}); 
      goto('/cart'); 
  ...
  function canCancelOrder(order: any) {
    ...
    if (fulfillmentStatus === 'shipped' || fulfillmentStatus === 'delivered' || fulfillmentStatus === 'fulfilled') {
      return false;
    }

    // Check if any fulfillments have been shipped
    if (order.fulfillments?.some((f: any) => f.shipped_at || f.status === 'shipped' || f.status === 'delivered')) {
      return false;
    }
  ...
  const tokenParam = urlParams.get('token');
    ...
    if (orderParam || emailParam || tokenParam) {
      errorMsg = null;
    }
  });
  ...
  $effect(() => {
    // Priority: token-only (no email required if token present)
    if (token && !order && !loading) {
      lookup();
      return;
    }
    // Fallback: email + order number combo
    if (email && orderNo && !order && !loading) lookup();
  });

  async function lookup() {
    errorMsg = null;
    order = null;
    // Allow token-only OR (email + orderNo) OR (email + token)
    ...
    loading = true;
    try {
      // Try lookup with display_id first, then fall back to token
      let found = null;
      if (orderNo && email) {
        found = await lookupOrder({ display_id: orderNo, email });
      }
      if (!found && token) {
        // Pass email only if we have it (lookupOrder ignores extra)
        found = await lookupOrder({ token, email: email || undefined });
      }
      if (!found && orderNo && !email) {
        // If user entered an orderNo but omitted email, attempt token path
  ...
  function shareLink() {
    ...
    if (order?.id && /^order_/.test(order.id)) return `${base}?token=${encodeURIComponent(order.id)}`;
    if (order?.display_id && order?.email) return `${base}?order=${encodeURIComponent(order.display_id)}&email=${encodeURIComponent(order.email)}`;
    return base;
  }
...
<svelte:head>
	...
	<meta name="description" content="Track your KhadkaFoods order status. Enter your order number and email to view order details, shipping status, and delivery updates." />
	<meta name="keywords" content="track order, order status, order lookup, shipping status, delivery tracking, order history" />
	<meta name="robots" content="noindex, follow" />
	<meta name="author" content="KhadkaFoods" />
	<meta property="og:title" content="Track Your Order • KhadkaFoods - Order Lookup" />
	<meta property="og:description" content="Track your KhadkaFoods order status. Enter your order number and email to view order details." />
	...
	<meta name="twitter:description" content="Track your KhadkaFoods order status. Enter your order number and email to view order details." />
...
<section class="w-full py-10">
  <div class="container mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
    ...
    <div class="card border border-base-300 bg-base-100 shadow-xl">
      <form class="card-body space-y-3" onsubmit={(e) => { e.preventDefault(); lookup(); }}>
        {#if errorMsg}
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          <label class="form-control">
            ...
            <input class="input input-bordered input-primary" type="email" bind:value={email} required />
          </label>
          <label class="form-control">
            <div class="label"><span class="label-text">Order number</span></div>
            <input class="input input-bordered input-primary" type="text" bind:value={orderNo} placeholder="e.g. 1024" />
          </label>
        </div>
        <div class="text-sm opacity-70">Or, if you received an order token link, paste the token below:</div>
        <label class="form-control">
          <input class="input input-bordered input-primary" type="text" bind:value={token} placeholder="Order token (optional)" />
        </label>
        <Button class={'btn btn-primary ' + (loading ? 'loading' : '')} disabled={loading} type="submit">Find order</Button>
      </form>
    </div>

    {#if loading && !order}
      <div class="mt-6 rounded-xl border border-base-300 bg-base-100 shadow-sm p-6 space-y-4 animate-pulse">
        <div class="h-4 w-40 bg-base-300 rounded"></div>
        <div class="flex gap-2">
          <div class="h-6 w-24 bg-base-300 rounded"></div>
          <div class="h-6 w-28 bg-base-300 rounded"></div>
          <div class="h-6 w-20 bg-base-300 rounded"></div>
        </div>
        <div class="h-32 w-full bg-base-200/70 rounded"></div>
        <div class="h-24 w-full bg-base-200/60 rounded"></div>
      </div>
    {/if}
    ...
    {#if order}
      <div class="mt-6">
        <Accordion.Root type="multiple" class="flex flex-col gap-4">
          <OrderCard {order} customerEmail={order.email} canCancel={canCancel} onCancel={cancelOrder} hydrate={hydrate} reorder={reorder} />
        </Accordion.Root>
      </div>
    {/if}
  </div>
