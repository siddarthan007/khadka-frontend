...
async function updatePromotions(method: 'POST' | 'DELETE', request: Request) {
	...
	const body = (await request.json().catch(() => ({}))) as { promo_codes?: string[] };
	const promo_codes = Array.isArray(body.promo_codes) ? body.promo_codes : [];
	if (!promo_codes.length) return json({ error: 'Missing promo_codes' }, { status: 400 });
	...
	try {
		...
		const message = e?.response?.data?.message || e?.message || 'Failed to update promotions';
