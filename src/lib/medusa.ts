...
export function logApiError(operation: string, error: any) {
  ...
  logger.error(`Medusa API Error (${operation}):`, errorMessage);
}
...
export function getStoreClient(): Medusa | null {
  ...
  if (!storeClient) {
    try {
      ...
      logger.warn(
        "Failed to initialize Medusa store client:",
...
let cachedRegion: { id: string; name: string; currency_code: string } | null =
  null;
...
export async function searchProducts(query: string, limit: number = 10) {
  try {
    ...
    if (!client) {
      logger.warn("MeiliSearch client not initialized");
      return [];
    }
    ...
    logger.error("MeiliSearch error:", error);
    return [];
  }
}
...
> {
  ...
  try {
    ...
    if (!store) return all;
    while (true) {
      const { product_categories = [], count = 0 } = await (
        store as any
      ).store.category.list({
        offset,
        limit,
...
export async function getHierarchicalProductCategories(): Promise<
  CategoryNode[]
> {
  try {
    ...
    for (const c of map.values()) {
      const parentId = (c as HttpTypes.StoreProductCategory)
        .parent_category_id as string | null | undefined;
...
const DEFAULT_PRODUCT_FIELDS =
  "id,handle,title,subtitle,description,created_at,thumbnail,*images,*variants.calculated_price,*variants.options,+variants.inventory_quantity,+variants.metadata,+variants.sku,*options,*options.values,*collection,*categories,*tags,+metadata";
...
export async function listProducts(
  params: Record<string, any> & ListProductsBaseOpts = {},
...
export async function listBasicProducts(
  ...
  try {
    const region = await resolveDefaultRegion();
    const fields =
      "id,handle,title,created_at,thumbnail,*images,variants.id,*variants.calculated_price";
...
export async function listProductsByCollectionId(
  ...
  const collectionFilter = Array.isArray(collectionId)
    ? collectionId
    : [collectionId];
  return listProducts({ collection_id: collectionFilter, ...opts });
}

export async function listProductsByCollectionHandle(
  handle: string,
  opts: ListProductsBaseOpts = {},
): Promise<ListProductsResult> {
  try {
...
export async function listProductsByCategoryIds(
...
export async function listProductsByCategoryHandles(
...
export async function lookupOrder(params: {
  ...
  try {
    ...
    if (token && /^order_/.test(token)) {
      try {
        const full = await store.store.order.retrieve(token, {
          fields: fullFields,
        });
    ...
    if (display_id && email) {
      try {
        const listResp = await store.store.order.list({
          fields:
            "id,display_id,email,status,payment_status,fulfillment_status,total,currency_code,created_at",
          limit: 50,
        });
        const match = (listResp as any)?.orders?.find(
          (o: any) =>
            String(o.display_id) === String(display_id) &&
            o.email?.toLowerCase() === email.toLowerCase(),
        );
        if (match) {
          try {
            const full = await store.store.order.retrieve(match.id, {
              fields: fullFields,
            });
    ...
    if (display_id && /^order_/.test(String(display_id))) {
      try {
        const full = await store.store.order.retrieve(String(display_id), {
          fields: fullFields,
        });
        return (full as any)?.order ?? null;
      } catch {}
...
export async function getOAuthCustomer(
  token: string,
  identity: string,
  ...
  try {
    ...
    if (!identity || typeof identity !== "string") {
      logger.warn(
        "[OAuth] getOAuthCustomer called without a valid auth_identity_id",
      );
      return null;
    }
