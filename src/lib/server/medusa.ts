...
export function getAdminClient(): Medusa | null {
  ...
  if (!adminClient) {
    try {
      ...
      logger.warn(
        "Failed to initialize Medusa admin client:",
...
export function getMeilisearchClient(): MeiliSearch | null {
  ...
  if (!meilisearchClient) {
    try {
      ...
      logger.warn(
        "Failed to initialize MeiliSearch (server):",
        (err as any)?.message ?? err,
      );
...
export async function getHeroSlides(): Promise<any[] | null> {
  try {
    const admin = getAdminClient();
    if (!admin) return [];
    const raw: Response = await admin.client.fetch("/admin/slides", {
      method: "GET",
    });
...
export async function searchProductsMeili(query: string, limit: number = 10) {
  try {
    ...
    if (!client) {
      logger.warn("MeiliSearch not configured");
      return [];
    }
