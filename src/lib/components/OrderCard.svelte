...
>
		...
		>
			...
			<div class="flex min-w-0 flex-1 flex-col gap-2 relative z-10">
				<div class="flex flex-wrap items-center gap-2">
					<span class="pr-2 font-mono text-sm leading-none font-semibold break-all sm:text-base text-base-content"
				...
				<div class="flex flex-col gap-1 text-xs text-base-content/70 sm:flex-row sm:gap-4">
					...
					>
						...
						<span class="text-sm font-semibold text-primary sm:text-base"
							>{formatCurrency(
								order.status === 'cancelled' || order.status === 'canceled'
	...
	<Accordion.Content class="bg-base-50/40 border-t border-base-300/70 p-3 md:p-4">
		<div class="flex flex-col gap-4 md:gap-6">
			...
			{#if order.status !== 'cancelled' && order.status !== 'canceled'}
				<div class="space-y-3">
					...
					<ol class="relative ml-2 flex flex-col gap-4 border-l-2 border-base-300/70">
						{#each filteredProgressSteps as step, idx}
							{#if step.reached(order)}
								<li class="relative pl-4 transition-all duration-300">
									<span
										class="absolute top-1.5 -left-[9px] h-4 w-4 rounded-full {step.key === 'cancelled' ? 'bg-error ring-error/20' : 'bg-primary ring-primary/20'} ring-4 transition-all duration-300 animate-pulse-slow"
									></span>
									<div class="text-sm font-medium {step.key === 'cancelled' ? 'text-error' : 'text-primary'} transition-colors duration-300">{step.label}</div>
									{#if stepTimestamp(order, step.key)}
										<div class="mt-0.5 text-[10px] opacity-60 transition-opacity duration-300">
											{stepTimestamp(order, step.key)}
										</div>
									{/if}
								</li>
							{:else}
								<li class="relative pl-4 opacity-40 transition-all duration-300">
									<span class="absolute top-1.5 -left-[9px] h-4 w-4 rounded-full bg-base-300 transition-all duration-300"
									></span>
									<div class="text-sm transition-colors duration-300">{step.label}</div>
								</li>
							{/if}
						{/each}
					</ol>
			...
			{#if order.items?.length}
				<div class="space-y-2">
					...
					<ul class="divide-y divide-base-300/70 rounded-xl border-2 border-base-300/50 bg-base-100 shadow-sm">
						{#each order.items as item (item.id)}
							<li class="grid grid-cols-12 gap-2 p-2 text-sm md:gap-3 md:p-3 transition-colors hover:bg-base-200/30">
									...
									<div class="truncate font-medium text-sm md:text-base">
										...
										{#if item.variant?.title}
											<span class="ml-1 text-xs font-normal text-base-content/60"
												>({escapeHtml(item.variant.title)})</span
											>
										{/if}
									</div>
								</div>
								...
								<div class="col-span-2 text-right font-semibold text-sm text-primary md:col-span-3 md:text-base">
			...
			<div class="space-y-2 md:space-y-1">
				...
				<div class="rounded-xl border-2 border-base-300/50 bg-base-100 p-3 shadow-sm">
			...
			{#if order.shipping_address}
				<div class="space-y-2">
					...
					<div class="rounded-xl border-2 border-base-300/50 bg-base-100 p-3 text-xs leading-relaxed shadow-sm">
						<div class="font-medium text-base-content">{escapeHtml(order.shipping_address.first_name || '')} {escapeHtml(order.shipping_address.last_name || '')}</div>
						{#if order.shipping_address.company}<div class="text-base-content/80">{escapeHtml(order.shipping_address.company)}</div>{/if}
						<div class="mt-1 text-base-content/80">{escapeHtml(order.shipping_address.address_1 || '')}</div>
						{#if order.shipping_address.address_2}<div class="text-base-content/80">{escapeHtml(order.shipping_address.address_2)}</div>{/if}
						<div class="text-base-content/80">
							{#if order.shipping_address.city && order.shipping_address.province}
								{escapeHtml(order.shipping_address.city)}, {escapeHtml(order.shipping_address.province)}
						...
						<div class="font-medium text-base-content/80">{escapeHtml(order.shipping_address.country_code?.toUpperCase() || '')}</div>
						{#if order.shipping_address.phone}<div class="mt-1 text-base-content/70">
					...
					<ul class="flex flex-col gap-2 text-xs">
						{#each order.payments || [] as p (p.id)}
							...
							>
								<span class="font-semibold text-base-content"
									>{escapeHtml((p.provider_id || p.provider || 'payment').replace(/_/g, ' '))}</span
								>
								{#if p.metadata?.card_brand || p.data?.card_brand}
									<span class="opacity-70"
										>{escapeHtml(p.metadata?.card_brand ||
						...
						{#each order.payment_collections || [] as pc (pc.id)}
							<li
								class="flex flex-wrap items-center gap-2 rounded-xl border-2 border-base-300/50 bg-base-100 px-3 py-2 shadow-sm transition-all hover:shadow-md"
							>
								<span class="font-semibold text-base-content">Stripe</span>
								<span class="opacity-70">{formatCurrency(pc.amount, order.currency_code)}</span>
								<span
									class="ml-auto rounded-lg bg-base-200 px-2 py-1 text-[10px] font-bold tracking-wide uppercase {pc.status === 'refunded' || pc.status === 'canceled'
										? 'text-error'
			...
			{#if order.refunds?.length || order.refunded_total > 0 || (order.status === 'cancelled' && order.total === 0)}
				<div class="space-y-2">
					...
					<div class="rounded-xl border-2 border-error/30 bg-error/5 p-3 text-xs space-y-1">
						{#each order.refunds || [] as r}
							<div class="flex justify-between items-center">
								<span class="text-base-content/80">{escapeHtml(r.reason || 'Refund')}</span>
								<span class="font-semibold text-error">{formatCurrency(r.amount, order.currency_code)}</span>
							</div>
						{/each}
						{#if order.refunded_total > 0}
							<div class="flex justify-between items-center pt-1 border-t border-error/20">
								<span class="font-medium text-base-content">Total refunded:</span>
								<span class="font-bold text-error">{formatCurrency(order.refunded_total, order.currency_code)}</span>
							</div>
			...
			<div class="flex flex-col gap-2 border-t-2 border-base-300/70 pt-3 sm:flex-row sm:flex-wrap sm:items-center">
				{#if canCancel(order)}
					<button 
						class="btn btn-outline btn-sm btn-error w-full sm:w-auto transition-all hover:scale-105 hover:shadow-lg" 
						onclick={() => {
							showCancelConfirm = true;
							trackEvent('order_cancel_initiated', { order_id: order.id });
						}}
					>
	...
	<dialog class="modal modal-open">
		<div class="modal-box rounded-2xl border-2 border-error/30 shadow-2xl">
			...
			<p class="py-4 text-base-content/80">
				Are you sure you want to cancel order <strong class="font-mono text-primary">#{order.display_id || order.id}</strong>?
				<br><br>
