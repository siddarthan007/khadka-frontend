...
<div
	class="double-range relative flex h-6 w-full items-center select-none"
	use:melt={$root}
	role="group"
	aria-label="Price range"
>
	<div class="relative h-[3px] w-full rounded-full bg-base-300">
		<div
			use:melt={$range}
	...
	{#each $thumbs as thumb, i}
		<span use:melt={thumb} class="absolute z-10 grid place-items-center">
			<span
				class="block h-5 w-5 rounded-full border-2 border-primary bg-base-100 shadow-md focus:outline focus:outline-2 focus:outline-offset-2 focus:outline-primary"
			></span>
			<span
